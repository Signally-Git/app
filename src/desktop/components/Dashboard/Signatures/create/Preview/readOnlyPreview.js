...
export default function ReadOnlyPreview({ template, infos }) {
    ...
    const mapObj = {
        PLACEHOLDER_COMPANY_ICON: infos?.logo || "https://via.placeholder.com/108",
        PLACEHOLDER_FIRST_NAME: infos?.firstName || "Prénom",
        PLACEHOLDER_LAST_NAME: infos?.lastName || "Nom",
        PLACEHOLDER_POSTE: infos?.jobName || "Poste",
    ...
    return parse(
        template.replace(
            /\b(?:PLACEHOLDER_FIRST_NAME|PLACEHOLDER_COMPANY_ICON|PLACEHOLDER_LAST_NAME|PLACEHOLDER_POSTE|PLACEHOLDER_COMPANY|PLACEHOLDER_ADDRESS|PLACEHOLDER_MOBILE|PLACEHOLDER_PHONE|PLACEHOLDER_EVENT_BANNER)\b/gi,
