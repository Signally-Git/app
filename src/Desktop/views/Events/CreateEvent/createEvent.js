...
export default function CreateEvent({ setDone, event }) {
    ...
    const checkEventLink = (url) => {
        ...
        if (isValid == 0)
            notification({
                content: (
                    <>
                        Le lien de l'évènement doit être formatté avec un
                        "https://liendelevenement"
                    </>
                ),
                status: "invalid",
            });
    ...
    const saveEvent = async (e) => {
        ...
        if (!eventName) {
            notification({
                content: <>Veuillez remplir le nom de l'event</>,
                status: "invalid",
            });
            return false;
        }
        ...
        if (!event && banner) {
            await request
                ...
                .catch((err) =>
                    notification({
                        content: <>Erreur lors de l'import de l'image</>,
                        status: "invalid",
                    })
                );
            return false;
        ...
        if (event) {
            ...
            if (banner) {
                ...
                await request
                    ...
                    .catch(() =>
                        notification({
                            content: <>Erreur lors de l'ajout de l'image</>,
                            status: "invalid",
                        })
                    );
                return false;
    ...
    return (
        <form onSubmit={(e) => saveEvent(e)} className={classes.container}>
            {event ? (
                <>
                    <h2>
                        Modifier event <span>{eventName}</span>
                    </h2>
                </>
            ) : (
                <>
