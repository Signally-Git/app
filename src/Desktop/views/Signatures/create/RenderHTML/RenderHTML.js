...
export default function RenderHTML(props) {
        ...
        await axios
            .get(
                `${process.env.REACT_APP_API_URL}/organisation/${
                    JSON.parse(localStorage.getItem("user"))?.organisation_id
    ...
    useEffect(() => {
        if (props.data.companyLogo)
            ...
            else setCompanyLogo(props.data.companyLogo);
    }, [props.data]);
    if (props.data?.firstName && props.template) {
        const mapObj = {
            PLACEHOLDER_BANNER:
                props.data.banner ||
                "https://images.signally.io/ec9128a1f3ad0413005f24e55eeef251a098e7e54e6",
            PLACEHOLDER_FIRST_NAME: props.data.firstName,
            PLACEHOLDER_FIRST_NAME_STYLE:
                props.data.firstNameStyle || "PLACEHOLDER_FIRST_NAME_STYLE",
            PLACEHOLDER_LAST_NAME: props.data.lastName,
            PLACEHOLDER_LAST_NAME_STYLE:
                props.data.lastNameStyle || "PLACEHOLDER_LAST_NAME_STYLE",
            PLACEHOLDER_POSITION: props.data.poste,
            PLACEHOLDER_POSTE_STYLE:
                props.data.posteStyle || "PLACEHOLDER_POSTE_STYLE",
            PLACEHOLDER_COMPANY: props.data.company || organisation?.name,
            PLACEHOLDER_COMPANY_STYLE:
                props.data.companyStyle || "PLACEHOLDER_COMPANY_STYLE",
