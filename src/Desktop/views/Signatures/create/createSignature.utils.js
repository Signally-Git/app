...
const getStyles = (signatureInfo, signatureOption, company, user) => {
    if (!signatureInfo)
        signatureInfo = defaultValues(company, user)
    return [
        // COLOR FOR EACH TXT
        {
            property: "color",
...
const handleSave = async (signatureName, selectedTemplate, company, signatureInfo, signatureOption, history, notification) => {
    await request
        .post(`signatures`, {
            name: signatureName,
