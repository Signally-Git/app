...
function OnBoarding({ organisation, completed, setCompleted }) {
    ...
    const handleSkip = () => {
        ...
        missingTabs[1] && menu.indexOf(missingTabs[1]) > slideIndex ? swiper?.slideTo(menu.indexOf(missingTabs[1])) :
            menu.indexOf(missingTabs[0]) > slideIndex ? swiper?.slideTo(menu.indexOf(missingTabs[0]))
                : setCompleted(true)
    }
        ...
        setMenu([
            {
                ...
                content: organisation.address?.street?.length > 0 ? <>Votre profil contient toutes les informations nécessaires aux templates des signatures.
                    ...
                    : <>Vous n'avez pas renseigné toutes les informations requises par certaines templates de signature ({missing.join(", ")})
                        <div className={classes.btnsContainer}>
                            <Button color="brown" onClick={() => handleSkip()}>Passer cette étape</Button>
                            <Button color="orange" onClick={() => history.push('/profile/informations/organisation')}>Compléter mon profil</Button>
                        </div>
                    </>
            },
            {
                ...
                content: <>
                    {organisation?.signatures?.length > 0 ?
                        ...
                        : <>Vous n'avez pas encore créé de signature. Cliquez ci-dessous pour en ajouter une !
                            <Button color="orange" onClick={() => history.push('/create-signature#onboarding')}>Créer ma première signature</Button>
                        </>}
            ...
            },
            {
                ...
                content: <>
                    {organisation?.events?.length > 1 ?
                        `Vous avez actuellement créé ${organisation?.events?.length} events.` : `Vous n'avez pas encore créé d'event. Cliquez ci-dessous pour en ajouter et programmer des campagnes de promotion`}
                    <div className={classes.btnsContainer}>
                        <Button color="brown" onClick={() => handleSkip()}>Passer cette étape</Button>
    ...
    if (completed === true)
        return (<div className={classes.container}>
            ...
            <Button color="orange" style={{ marginRight: 'auto', marginLeft: 0 }} onClick={() => setCompleted('show')}>Relancer l'onboarding</Button>
        </div>)
    return (<div className={classes.container}>
        <h5>
            {menu[slideIndex]?.title}
        </h5>
        <div className={classes.swiperContainer}>
            <ul className={classes.menuLinks}>
                {menu.map((element, index) => {
                    return <li onClick={() => swiper?.slideTo(index)} key={index} className={`${slideIndex === index ? classes.active : ""} ${element.done ? classes.disabled : classes.enabled}`}>
                        {element.link} {element.done ? <ImCheckmark /> : ""}
                    </li>
                })}
            </ul>
            <Swiper
