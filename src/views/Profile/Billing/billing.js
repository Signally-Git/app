    ...
    useEffect(async () => {
        ...
        await axios
            .get(
                ...
                }/users?access_token=${localStorage.getItem("token")}`
            )
            .then((res) => {
            ...
            <div className={classes.greyContainer}>
                <div className={classes.whiteContainer}>
                    <div className={classes.textContainer}>
                        ...
                        {users.length > 1 ? (
                            <>
                                <h4>{users?.length + " utilisateurs"}</h4>{" "}
                                <p>0,5 € / mois / utilisateur</p>
                            </>
                        ) : (
                            <>
                                <h4>{users?.length + " utilisateur"}</h4>{" "}
                                <p>Gratuit</p>
                            </>
                        )}
                    ...
                    <div className={classes.priceContainer}>
                        {users.length === 1 && bannerActivesAPI === 0 ? (
                            <>
                                <span className={classes.bigTxt}> </span>
